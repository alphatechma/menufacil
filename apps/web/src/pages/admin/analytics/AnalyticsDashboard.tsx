...
// Tooltip Recharts ciente do tema (igual ao Dashboard) — usa tokens do tema, adapta ao dark mode.
const tooltipStyle = {
  borderRadius: '12px',
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            className="px-3 py-2 border border-border bg-background text-foreground rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
          />
        </div>
        <div className="flex items-center gap-2">
...
function KpiCard({ title, icon: Icon, current, previous, change, format = 'number' }: KpiCardProps) {
  ...
  return (
    <div className="bg-card rounded-2xl shadow-sm border border-border p-5">
      ...
      <p className="text-2xl font-bold text-foreground">{formatValue(current)}</p>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-card rounded-2xl shadow-sm border border-border p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-semibold text-foreground">Receita por Dia</h3>
          <button
            onClick={onExport}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-end">
        <button
          onClick={onExport}
          className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-medium text-muted-foreground bg-card border border-border hover:bg-muted transition-colors active:scale-95"
        >
      ...
      {/* Top Products Table */}
      <div className="bg-card rounded-2xl shadow-sm border border-border p-6">
        <h3 className="text-sm font-semibold text-foreground mb-4">Top 10 Produtos</h3>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {data.topProducts?.map((p: any, i: number) => (
                <tr key={p.id || i} className="border-b border-border hover:bg-muted/50">
                  <td className="py-3 px-2 font-medium text-foreground">{p.name}</td>
                  <td className="py-3 px-2 text-muted-foreground">{p.category}</td>
                  <td className="py-3 px-2 text-right text-foreground">{p.quantity}</td>
      ...
      <div className="bg-card rounded-2xl shadow-sm border border-border p-6">
        <h3 className="text-sm font-semibold text-foreground mb-4">Receita por Categoria</h3>
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data.categoryBreakdown} layout="vertical">
              <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
              <XAxis type="number" tickFormatter={(v) => formatCurrency(v)} stroke="hsl(var(--muted-foreground))" tick={{ fontSize: 11, fill: 'hsl(var(--muted-foreground))' }} />
              <YAxis type="category" dataKey="category" stroke="hsl(var(--muted-foreground))" tick={{ fontSize: 11, fill: 'hsl(var(--muted-foreground))' }} width={120} />
      ...
      {/* Profitability Table */}
      <div className="bg-card rounded-2xl shadow-sm border border-border p-6">
        <h3 className="text-sm font-semibold text-foreground mb-4">Lucratividade</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                ...
                <tr key={p.id || i} className="border-b border-border hover:bg-muted/50">
                  ...
                  <td
                    className={cn(
                      ...
                      p.marginPercent < 30 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400',
                    )}
                  >
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-6">
          ...
          <div>
            <p className="text-xs text-muted-foreground">Frequencia Media</p>
            <p className="text-xl font-bold text-foreground">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data.newVsReturning}>
              ...
              <XAxis
                ...
                tick={{ fontSize: 11, fill: 'hsl(var(--muted-foreground))' }}
              />
              <YAxis stroke="hsl(var(--muted-foreground))" tick={{ fontSize: 11, fill: 'hsl(var(--muted-foreground))' }} />
              <Tooltip
                contentStyle={tooltipStyle}
                labelFormatter={(label) => {
                  const d = parseDayDate(label);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Top Customers */}
        <div className="bg-card rounded-2xl shadow-sm border border-border p-6">
          <h3 className="text-sm font-semibold text-foreground mb-4">Top 10 Clientes</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
              ...
              <tbody>
                {data.topCustomers?.map((c: any, i: number) => (
                  <tr key={c.id || i} className="border-b border-border hover:bg-muted/50">
                    <td className="py-3 px-2 font-medium text-foreground">{c.name}</td>
                    <td className="py-3 px-2 text-right text-foreground">{c.orderCount}</td>
                    <td className="py-3 px-2 text-right font-medium text-foreground">
        ...
        {/* RFM Segments Donut */}
        <div className="bg-card rounded-2xl shadow-sm border border-border p-6">
          <h3 className="text-sm font-semibold text-foreground mb-4">Segmentos RFM</h3>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
            ...
            {segmentData.map((seg) => (
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{seg.value}</p>
                </div>
              </div>
            ))}
          </div>
...
function DeliveryTab({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-card rounded-2xl shadow-sm border border-border p-6">
        <h3 className="text-sm font-semibold text-foreground mb-4">Entregadores</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
            ...
            <tbody>
              {data.deliveryPersons?.map((dp: any, i: number) => (
                <tr key={dp.id || i} className="border-b border-border hover:bg-muted/50">
                  <td className="py-3 px-2 font-medium text-foreground">{dp.name}</td>
                  <td className="py-3 px-2 text-right text-foreground">{dp.deliveries}</td>
                  <td className="py-3 px-2 text-right text-foreground">
            ...
            <tbody>
              {data.zoneStats?.map((z: any, i: number) => (
                <tr key={i} className="border-b border-border hover:bg-muted/50">
                  <td className="py-3 px-2 font-medium text-foreground">{z.zone}</td>
                  <td className="py-3 px-2 text-right text-foreground">{z.deliveries}</td>
                  <td className="py-3 px-2 text-right text-foreground">
