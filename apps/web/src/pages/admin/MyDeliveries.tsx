  ...
  if (!myDeliveryPerson) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <h2 className="text-xl font-bold text-gray-700 mb-2">Entregador nao vinculado</h2>
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          <div className="text-center px-4 py-2 bg-white border border-gray-200 rounded-xl">
            ...
            <p className="text-[10px] text-gray-500 font-medium uppercase tracking-wider">Ativa(s)</p>
          </div>
          <div className="text-center px-4 py-2 bg-white border border-gray-200 rounded-xl">
            <p className="text-2xl font-bold text-green-600">{completedDeliveries.length}</p>
            <p className="text-[10px] text-gray-500 font-medium uppercase tracking-wider">Entregues</p>
          </div>
        </div>
      </div>
      ...
      {hasCommission && earnings && (
        <div className="bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-200 rounded-2xl p-5 mb-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="font-bold text-green-800">Seus Ganhos</h2>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white rounded-xl p-4 border border-green-100">
              <div className="flex items-center gap-2 mb-1">
                ...
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Hoje</p>
              </div>
              <p className="text-2xl font-black text-green-700">{formatPrice(earnings.todayEarnings)}</p>
              <p className="text-xs text-gray-500 mt-0.5">{earnings.todayCount} entrega(s)</p>
            </div>
            <div className="bg-white rounded-xl p-4 border border-green-100">
              <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-2xl font-black text-emerald-700">{formatPrice(earnings.totalEarnings)}</p>
              <p className="text-xs text-gray-500 mt-0.5">{earnings.totalCount} entrega(s)</p>
            </div>
          </div>
        </div>
      )}
      ...
      {activeDeliveries.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-600 mb-1">Nenhuma entrega ativa</h3>
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                        isOutForDelivery ? 'bg-blue-100' : 'bg-amber-100'
                      }`}
                    >
                      {isOutForDelivery ? (
                    ...
                    <div>
                      ...
                      <p
                        className={`text-xs font-semibold ${
                          isOutForDelivery ? 'text-blue-600' : 'text-amber-600'
                        }`}
                      >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className={`text-sm font-bold ${time.minutes >= 30 ? 'text-red-600' : time.minutes >= 15 ? 'text-amber-600' : 'text-gray-600'}`}>
                        {time.text}
                      </span>
                    </div>
                  </div>
                    ...
                    <div className="flex items-start gap-3 bg-gray-50 rounded-xl p-3">
                      ...
                      <div className="text-sm text-gray-600">
                        ...
                        {order.address_snapshot.complement && (
                          <p className="text-xs text-gray-500">{order.address_snapshot.complement}</p>
                        )}
              ...
              <div className="bg-green-50 px-5 py-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">Pedido #{order.order_number || order.id?.slice(0, 6)}</p>
                  </div>
                </div>
                <button
                  onClick={() => setConfirmingOrderId(null)}
                  className="p-1.5 rounded-lg hover:bg-green-100 text-gray-400 transition-colors"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              <div className="px-5 py-4">
                ...
                <div className="bg-gray-50 rounded-xl p-3 mb-4 space-y-2">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="font-medium text-gray-800">{order.customer?.name || 'Cliente'}</span>
                  </div>
                  {order.address_snapshot && (
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="font-bold text-gray-800">{formatPrice(order.total || 0)}</span>
                  </div>
                </div>
      ...
      {completedDeliveries.length > 0 && (
        <div className="mt-8">
          <button
            ...
            className="flex items-center gap-2 text-sm font-semibold text-gray-600 mb-3 hover:text-gray-800"
          >
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-right">
                      <p className="text-sm font-bold text-gray-700">{formatPrice(order.total || 0)}</p>
                      {commission > 0 && (
                        <p className="text-xs font-bold text-green-600">+{formatPrice(commission)}</p>
                      )}
                      {order.delivered_at && (
