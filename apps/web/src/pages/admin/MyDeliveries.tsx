  ...
  if (!myDeliveryPerson) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <h2 className="text-xl font-bold text-gray-700 dark:text-gray-200 mb-2">Entregador nao vinculado</h2>
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          <div className="text-center px-4 py-2 bg-white dark:bg-slate-800 border border-gray-200 dark:border-gray-700 rounded-xl">
            <p className="text-2xl font-bold text-primary">{activeDeliveries.length}</p>
            <p className="text-[10px] text-gray-500 dark:text-gray-400 font-medium uppercase tracking-wider">Ativa(s)</p>
          </div>
          <div className="text-center px-4 py-2 bg-white dark:bg-slate-800 border border-gray-200 dark:border-gray-700 rounded-xl">
            <p className="text-2xl font-bold text-green-600">{completedDeliveries.length}</p>
            <p className="text-[10px] text-gray-500 dark:text-gray-400 font-medium uppercase tracking-wider">Entregues</p>
          </div>
        </div>
      </div>
      ...
      {hasCommission && earnings && (
        <div className="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/10 dark:to-emerald-900/10 border-2 border-green-200 dark:border-green-800 rounded-2xl p-5 mb-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-green-100 dark:border-green-800">
              <div className="flex items-center gap-2 mb-1">
                <Clock className="w-4 h-4 text-green-500" />
                <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Hoje</p>
              </div>
              <p className="text-2xl font-black text-green-700 dark:text-green-400">{formatPrice(earnings.todayEarnings)}</p>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{earnings.todayCount} entrega(s)</p>
            </div>
            <div className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-green-100 dark:border-green-800">
              <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{earnings.totalCount} entrega(s)</p>
            </div>
          </div>
        </div>
      )}
      ...
      {activeDeliveries.length === 0 ? (
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-12 text-center">
          <Package className="w-16 h-16 text-gray-200 dark:text-gray-600 mx-auto mb-4" />
          <h3 className="text-lg font-semibold text-gray-600 dark:text-gray-300 mb-1">Nenhuma entrega ativa</h3>
        ...
        <div className="space-y-4">
          {activeDeliveries.map((order: any) => {
            ...
            return (
              <div
                key={order.id}
                className={`bg-white dark:bg-slate-800 rounded-2xl border-2 overflow-hidden transition-all ${
                  isOutForDelivery
              ...
              >
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className={`text-sm font-bold ${time.minutes >= 30 ? 'text-red-600' : time.minutes >= 15 ? 'text-amber-600' : 'text-gray-600 dark:text-gray-300'}`}>
                        {time.text}
                      </span>
                    </div>
                  </div>
                ...
                <div className="px-5 py-2 space-y-3">
                  {order.address_snapshot && (
                    <div className="flex items-start gap-3 bg-gray-50 dark:bg-slate-700/50 rounded-xl p-3">
                      ...
                      <div className="text-sm text-gray-600 dark:text-gray-300">
                        ...
                        {order.address_snapshot.complement && (
                          <p className="text-xs text-gray-500 dark:text-gray-400">{order.address_snapshot.complement}</p>
                        )}
            ...
            <div className="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
              <div className="bg-green-50 dark:bg-green-900/20 px-5 py-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-gray-900 dark:text-gray-100">Confirmar Entrega</h3>
                    <p className="text-xs text-gray-500 dark:text-gray-400">Pedido #{order.order_number || order.id?.slice(0, 6)}</p>
                  </div>
                </div>
                <button
                  onClick={() => setConfirmingOrderId(null)}
                  className="p-1.5 rounded-lg hover:bg-green-100 dark:hover:bg-green-900/30 text-gray-400 dark:text-gray-500 transition-colors"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              <div className="px-5 py-4">
                ...
                <div className="bg-gray-50 dark:bg-slate-700/50 rounded-xl p-3 mb-4 space-y-2">
                  <div className="flex items-center gap-2 text-sm">
                    <User className="w-4 h-4 text-gray-400 dark:text-gray-500" />
                    <span className="font-medium text-gray-800 dark:text-gray-100">{order.customer?.name || 'Cliente'}</span>
                  </div>
                  {order.address_snapshot && (
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="font-bold text-gray-800 dark:text-gray-100">{formatPrice(order.total || 0)}</span>
                  </div>
                </div>
                <div className="flex gap-3">
                  <button
                    onClick={() => setConfirmingOrderId(null)}
                    className="flex-1 py-3 rounded-xl border-2 border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 font-semibold text-sm hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors"
                  >
                    Cancelar
                  </button>
                  <button
      ...
      {completedDeliveries.length > 0 && (
        <div className="mt-8">
          <button
            ...
            className="flex items-center gap-2 text-sm font-semibold text-gray-600 dark:text-gray-300 mb-3 hover:text-gray-800 dark:hover:text-gray-100"
          >
            ...
            <div className="space-y-2">
              {completedDeliveries.map((order: any) => {
                ...
                return (
                  <div
                    key={order.id}
                    className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-gray-700 px-4 py-3 flex items-center justify-between"
                  >
                    <div className="flex items-center gap-3">
                    ...
                    <div className="text-right">
                      <p className="text-sm font-bold text-gray-700 dark:text-gray-200">{formatPrice(order.total || 0)}</p>
                      {commission > 0 && (
