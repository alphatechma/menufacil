...
function getUrgencyStyle(minutes: number): string {
  if (minutes >= 45) return 'border-red-300 bg-red-50';
  if (minutes >= 30) return 'border-amber-300 bg-amber-50';
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <div className="text-sm font-medium text-gray-600 bg-white border border-gray-200 px-4 py-2 rounded-xl">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <Card className="p-4 text-center">
          ...
          <p className="text-xs text-gray-500 font-medium uppercase tracking-wider">Em Entrega</p>
        </Card>
        <Card className="p-4 text-center">
          <div className="w-10 h-10 rounded-xl bg-green-50 text-green-600 flex items-center justify-center mx-auto mb-2">
            <Clock className="w-5 h-5" />
          </div>
          <p className="text-2xl font-bold text-gray-900">{formatMinutes(avgDeliveryTime)}</p>
          <p className="text-xs text-gray-500 font-medium uppercase tracking-wider">Tempo Medio</p>
        </Card>
        <Card className="p-4 text-center">
          <div className="w-10 h-10 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center mx-auto mb-2">
            <Zap className="w-5 h-5" />
          </div>
          <p className="text-2xl font-bold text-gray-900">{formatMinutes(perfStats?.fastest_order ?? 0)}</p>
          <p className="text-xs text-gray-500 font-medium uppercase tracking-wider">Mais Rapido</p>
        </Card>
        <Card className="p-4 text-center">
          ...
          <p className="text-xs text-gray-500 font-medium uppercase tracking-wider">Entregues (7d)</p>
        </Card>
      </div>
      ...
      <div className="mb-8">
        ...
        {activeDeliveries.length === 0 ? (
          <Card className="p-12 text-center">
            ...
            <p className="text-gray-500 font-medium">Nenhum pedido em entrega no momento</p>
            <p className="text-sm text-gray-400 mt-1">Pedidos apareceraoaqui quando forem enviados para entrega</p>
          </Card>
        ) : (
            ...
            {activeDeliveries.map((order: any) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Items summary */}
                  <div className="bg-white/70 rounded-xl px-3 py-2 mb-3 border border-gray-100">
                    <p className="text-xs text-gray-500 font-medium mb-1">{order.items?.length || 0} item(s)</p>
                    <div className="space-y-0.5">
                      ...
                      {order.items?.length > 3 && (
                        <p className="text-xs text-gray-400">+{order.items.length - 3} mais</p>
                      )}
                    </div>
                    <p className="text-sm font-semibold text-gray-900 mt-1 pt-1 border-t border-gray-100">
                  ...
                  {order.delivery_person && (
                    <div className="flex items-center gap-2 text-sm bg-primary/5 border border-primary/10 rounded-xl px-3 py-2 mb-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-medium text-gray-900 text-sm">{order.delivery_person.name}</p>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-100">
                    ...
                    <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Tempo de Entrega</th>
                    <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Entregue em</th>
                  </tr>
                </thead>
